...
class ManagedPosition:
    ...
    def log_event(self, event: str):
        ...
        log.info("📍 {} | {}", self.trade.symbol, event)
...
class AdaptiveRiskManager:
    ...
    async def analyse(self, df: pd.DataFrame, symbol: str, timeframe: str) -> MarketConditions:
        ...
        log.info("📊 MarketConditions | {} {} | {}", symbol, timeframe, cond.to_log_str())
        return cond
    ...
    def _classify_regime(self, c: MarketConditions) -> MarketRegime:
        if c.atr_ratio > 2.0:       return MarketRegime.HIGH_VOLATILE
    ...
    def _calc_risk_multiplier(self, c: MarketConditions) -> float:
        base = {
            MarketRegime.STRONG_TREND:  1.30,
            MarketRegime.WEAK_TREND:    1.00,
            MarketRegime.RANGING:       0.70,
            MarketRegime.HIGH_VOLATILE: 0.50,
            MarketRegime.LOW_VOLATILE:  0.80,
            MarketRegime.BREAKOUT:      1.10,
        }.get(c.regime, 1.0)
        if c.rsi > 75 or c.rsi < 25: base *= 0.85
        if c.volume_ratio > 1.5:      base *= 1.10
        return round(min(max(base, 0.3), 1.5), 2)
...
class PositionSizer:
    ...
    def calculate(
        ...
        cfg = settings.risk
        base_risk_pct  = cfg.max_risk_per_trade_pct / 100
        ...
        lot = round(min(max(lot, 0.01), cfg.max_lot_size), 2)
        ...
        log.info(
            "💰 PositionSizer | {} {} | {}/{} | Lot={} | Risk={:.2f}%(${:.2f}) | SLdist={:.5f}",
...
class TradeExecutor:
    ...
    async def execute(
        ...
        log.success("TRADE EXECUTED | {}", record.to_log_line())
...
class PositionTracker:
    ...
    async def add(self, pos: ManagedPosition):
        async with self._lock:
            ...
            log.info("📌 Tracking {} {} (id={})", pos.trade.direction, pos.trade.symbol, pos.trade.id[:8])

    async def remove(self, trade_id: str) -> Optional[ManagedPosition]:
        async with self._lock:
            pos = self._positions.pop(trade_id, None)
            if pos:
                log.info("🗑️ Untracked {} ({})", pos.trade.symbol, trade_id[:8])
            return pos
...
class TechnicalAnalyser:
    ...
    def _ai_momentum(self, df, symbol, tf, quote, conditions: MarketConditions) -> Optional[TradingSignal]:
        ...
        return TradingSignal(
            ...
            metadata={"regime":conditions.regime.value,"adx":conditions.adx,
                      "atr":conditions.atr,"risk_multiplier":conditions.risk_multiplier},
        )
    ...
    def _mean_reversion(self, df, symbol, tf, quote, conditions: MarketConditions) -> Optional[TradingSignal]:
        ...
        return TradingSignal(
            ...
            metadata={"regime":conditions.regime.value,"bb_width":conditions.bb_width},
        )
    ...
    def _fallback_signal(self, df, symbol, tf, quote, strategy, conditions) -> Optional[TradingSignal]:
        ...
        return TradingSignal(
            ...
            metadata={"regime": conditions.regime.value},
        )
