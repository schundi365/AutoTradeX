...
- Runs a local fine-tuned Ollama LLM (Llama 3.1 8B) as primary brain for SMC and macro reasoning
- Persists all data in DuckDB (analytics) and ChromaDB (news/trade context RAG)
...
│
├── scripts/
│   ├── prepare_training_data.py     ← export DuckDB trades as LLM-ready JSONL
│   ├── finetune.py                  ← Unsloth QLoRA fine-tuning script
│   ├── conversion.py                ← llama.cpp GGUF conversion instructions
│   ├── Ollamamodefile.py            ← Ollama Modelfile preparation
│   ├── wire_bot.py                  ← Tiered LLM client implementation
│   ├── debug_mt5_alive.py           ← Connectivity check
...
Tier 3: Claude Haiku via Anthropic SDK
  ...
  → Timeout: 15s
  → if all tiers fail: return fallback decision (score > 8.0)
```
...
- Use LLM (via tiered client) to generate reasoning text for top signals
- Apply ReAct pattern: Thought → Action → Observation → Conclusion
- Output structured JSON: {"confidence_score": 0.0-10.0, "reasoning": "..."}
...
### scripts/finetune.py
- Use Unsloth + QLoRA (4-bit quantization) for maximum memory efficiency
- Base model: `unsloth/llama-3.1-8b-bnb-4bit`
- LoRA parameters: R=16, alpha=16, target_modules="all-linear"
- Training: 3 epochs, AdamW 8-bit optimizer
- Output: `apex_trading_model` (HF weights)

### scripts/conversion.py & Ollamamodefile.py
- Convert HF weights to GGUF using `llama.cpp` (quantization: `q4_k_m`)
- Create `Modelfile` with explicit SYSTEM prompt (SMC specialty, macro focus)
- Load into Ollama: `ollama create apex-trader -f Modelfile`
